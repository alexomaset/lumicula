...
interface DosAndDontsProps {
  ...
  onUpdateDosOrDonts: (type: 'dos' | 'donts', index: number, value: string) => void;
}
  ...
  const renderSection = (type: 'dos' | 'donts', items: string[]) => (
    <div>
      <div className="flex justify-between items-center mb-2">
        <h3 className="text-lg font-semibold">{type === 'dos' ? 'Dos' : "Don'ts"}</h3>
        <Button 
          type="button" 
          variant="outline" 
          size="sm" 
        ...
        >
          <Plus className="mr-2 h-4 w-4" /> Add {type === 'dos' ? 'Do' : "Don't"}
        </Button>
      </div>
